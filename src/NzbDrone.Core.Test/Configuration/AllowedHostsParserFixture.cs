...
namespace NzbDrone.Core.Test.Configuration
{
    ...
    public class AllowedHostsParserFixture : TestBase
    {
        ...
        {
            var result = AllowedHostsParser.Parse("lidarr.local, 192.168.1.5; myhost");

            result.Should().BeEquivalentTo("lidarr.local", "192.168.1.5", "myhost");
        }
        ...
        [TestCase("lidarr.local")]
        [TestCase("lidarr.example.co.uk")]
        [TestCase("my-host")]
        ...
        [TestCase("")]
        [TestCase("  ")]
        [TestCase("http://lidarr.local")]
        [TestCase("lidarr.local:8989")]
        [TestCase("lidarr local")]
        [TestCase("lidarr.*")]
        [TestCase("*")]
        [TestCase("*.")]
