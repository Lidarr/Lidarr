...
namespace NzbDrone.Core.Download.Clients.FreeboxDownload
{
    public class TorrentFreeboxDownload : TorrentClientBase<FreeboxDownloadSettings>
    {
        ...
        private bool ToBeQueuedFirst(RemoteAlbum remoteAlbum)
        {
            if ((remoteAlbum.IsRecentAlbum() && Settings.RecentPriority == (int)FreeboxDownloadPriority.First) ||
                (!remoteAlbum.IsRecentAlbum() && Settings.OlderPriority == (int)FreeboxDownloadPriority.First))
            {
                return true;
            }
        ...
        private double? GetSeedRatio(RemoteAlbum remoteAlbum)
        {
            if (remoteAlbum.SeedConfiguration == null || remoteAlbum.SeedConfiguration.Ratio == null)
            {
                return null;
            }

            return remoteAlbum.SeedConfiguration.Ratio.Value * 100;
        }
    }
}
